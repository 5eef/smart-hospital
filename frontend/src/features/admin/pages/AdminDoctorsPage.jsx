...
export function AdminDoctorsPage() {
  ...
  function edit(doctor) { setEditing(doctor.id); setFormErrors({}); setForm({ name: doctor.user?.name ?? '', email: doctor.user?.email ?? '', phone: doctor.user?.phone ?? '', department_id: doctor.department_id ?? '', license_number: doctor.license_number ?? '', specialty: doctor.specialty ?? '', status: doctor.status ?? 'active', is_active: doctor.user?.is_active ?? true }); setFormOpen(true) }
  async function save(event) { event.preventDefault(); if (isSaving) return; setIsSaving(true); setFormErrors({}); try { if (editing) await resourceService.update('doctors', editing, form); else await resourceService.create('doctors', form); setFormOpen(false); setEditing(null); setForm(emptyForm); await refetch(); notify('Médecin enregistré.') } catch (requestError) { setFormErrors(requestError.response?.data?.errors ?? {}); notify(apiError(requestError, "Impossible d’enregistrer le médecin.")) } finally { setIsSaving(false) } }
  async function confirmAction() { if (!pendingAction || isActionLoading) return; setIsActionLoading(true); try { if (pendingAction.type === 'delete') { await resourceService.remove('doctors', pendingAction.id); await refetch(); notify('Médecin supprimé.') } else { await api.post(`/doctors/${pendingAction.id}/reset-password`); notify('Mot de passe réinitialisé avec succès.') } setPendingAction(null) } catch (requestError) { notify(apiError(requestError, pendingAction.type === 'delete' ? 'Suppression impossible.' : 'Réinitialisation impossible.')) } finally { setIsActionLoading(false) } }
  ...
  return <div className="page-stack">
    ...
    <section className="panel overflow-hidden">
      <div className="flex flex-col gap-3 border-b p-5 lg:flex-row lg:items-center lg:justify-between" style={{ borderColor: 'var(--border)' }}><SearchField value={search} onChange={(event) => { const next = new URLSearchParams(searchParams); if (event.target.value) next.set('search', event.target.value); else next.delete('search'); setSearchParams(next, { replace: true }); setPage(1) }} placeholder="Rechercher par nom, spécialité ou email..." className="max-w-xl" /><select value={department} onChange={(event) => { setDepartment(event.target.value); setPage(1) }} className="field lg:w-64"><option value="">Toutes les spécialités</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[950px] text-left"><thead className="table-head text-xs uppercase tracking-widest"><tr>{['Nom', 'Spécialité', 'Contact', 'Statut', 'Actions'].map((head) => <th key={head} className="px-6 py-4">{head}</th>)}</tr></thead><tbody>{isLoading ? <tr><td colSpan="5" className="muted px-6 py-8">Chargement...</td></tr> : doctors.length ? doctors.map((doctor) => <tr key={doctor.id} className="table-row border-t transition"><td className="px-6 py-4"><div className="flex items-center gap-3"><Avatar name={userName(doctor)} /><div><p className="font-semibold">{userName(doctor)}</p><p className="muted text-xs">ID: #DOC-{doctor.id}</p></div></div></td><td className="px-6 py-4"><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-950/50 dark:text-blue-300">{doctor.specialty || doctor.department?.name}</span></td><td className="px-6 py-4"><p>{doctor.user?.email}</p><p className="muted text-sm">{doctor.user?.phone || '—'}</p></td><td className="px-6 py-4"><StatusBadge status={doctor.status} /></td><td className="px-6 py-4"><div className="flex gap-1"><button className="icon-button" onClick={() => edit(doctor)} disabled={isActionLoading} type="button" aria-label={`Modifier ${userName(doctor)}`} title="Modifier"><Pencil size={18} /></button><button className="icon-button" onClick={() => setPendingAction({ type: 'reset', id: doctor.id, name: userName(doctor) })} disabled={isActionLoading} type="button" aria-label={`Réinitialiser le mot de passe de ${userName(doctor)}`} title="Réinitialiser le mot de passe"><KeyRound size={18} /></button><button className="icon-button hover:!text-red-600" onClick={() => setPendingAction({ type: 'delete', id: doctor.id, name: userName(doctor) })} disabled={isActionLoading} type="button" aria-label={`Supprimer ${userName(doctor)}`} title="Supprimer"><Trash2 size={18} /></button></div></td></tr>) : <tr><td colSpan="5" className="muted px-6 py-10 text-center">Aucun médecin.</td></tr>}</tbody></table></div>
      <Pagination pagination={pagination} onPageChange={setPage} />
    </section>
    <Modal open={formOpen} onClose={isSaving ? () => {} : () => setFormOpen(false)} title={editing ? 'Modifier le médecin' : 'Ajouter un médecin'} description="Les informations sont enregistrées dans le compte et le profil médical existants."><form onSubmit={save} className="grid gap-4 sm:grid-cols-2">{[['name', 'Nom complet', 'text'], ['email', 'Adresse email', 'email'], ['phone', 'Téléphone', 'tel'], ['license_number', 'Numéro de licence', 'text'], ['specialty', 'Spécialité affichée', 'text']].map(([field, label, type]) => <label key={field} className="field-label">{label}<input required={['name', 'email', 'license_number', 'specialty'].includes(field)} type={type} value={form[field]} onChange={(event) => setForm({ ...form, [field]: event.target.value })} className="field" aria-invalid={Boolean(formErrors[field])} />{formErrors[field] ? <span className="text-xs font-normal text-red-600">{Array.isArray(formErrors[field]) ? formErrors[field][0] : formErrors[field]}</span> : null}</label>)}<label className="field-label">Département<select required value={form.department_id} onChange={(event) => setForm({ ...form, department_id: event.target.value })} className="field" aria-invalid={Boolean(formErrors.department_id)}><option value="">Choisir</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{formErrors.department_id ? <span className="text-xs font-normal text-red-600">{formErrors.department_id[0]}</span> : null}</label><label className="field-label">Statut<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })} className="field"><option value="active">Actif</option><option value="inactive">Inactif</option><option value="leave">En congé</option></select></label><label className="flex items-center gap-3 pt-7 text-sm font-semibold"><input type="checkbox" checked={form.is_active} onChange={(event) => setForm({ ...form, is_active: event.target.checked })} />Compte utilisateur actif</label><div className="flex justify-end gap-3 sm:col-span-2"><Button variant="secondary" onClick={() => setFormOpen(false)} disabled={isSaving}>Annuler</Button><Button type="submit" disabled={isSaving}><Save size={18} />{isSaving ? 'Enregistrement...' : 'Enregistrer'}</Button></div></form></Modal>
    <ConfirmModal open={Boolean(pendingAction)} onClose={() => setPendingAction(null)} onConfirm={confirmAction} isLoading={isActionLoading} title={pendingAction?.type === 'delete' ? 'Supprimer le médecin' : 'Réinitialiser le mot de passe'} description={pendingAction?.type === 'delete' ? `Confirmez la suppression de ${pendingAction?.name}. Cette action supprimera aussi son compte.` : `Le mot de passe de ${pendingAction?.name} sera réinitialisé à la valeur administrative prévue.`} confirmLabel={pendingAction?.type === 'delete' ? 'Supprimer' : 'Réinitialiser'} />
  </div>
}
