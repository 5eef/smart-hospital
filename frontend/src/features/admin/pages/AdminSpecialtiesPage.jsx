...
export function AdminSpecialtiesPage() {
  ...
  async function save(event) { event.preventDefault(); try { if (editing) await resourceService.update('departments', editing, form); else await resourceService.create('departments', form); setFormOpen(false); setEditing(null); setForm(emptyForm); await refetch(); notify('Spécialité enregistrée.') } catch (requestError) { notify(apiError(requestError, "Impossible d’enregistrer la spécialité.")) } }
  async function remove(id) { if (!window.confirm('Supprimer cette spécialité ?')) return; try { await resourceService.remove('departments', id); await refetch(); notify('Spécialité supprimée.') } catch (requestError) { notify(apiError(requestError, 'Suppression impossible si des médecins ou rendez-vous y sont liés.')) } }
